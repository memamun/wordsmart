    ...
    <div style={{ padding: '2rem', maxWidth: '850px', margin: '0 auto' }} className="animate-fade stories-view-container">
              ...
              <div key={idx} className="card card-hover" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', justifyBetween: 'space-between' }}>
                <div>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'hsl(var(--primary))', fontWeight: '700', display: 'block', marginTop: '0.25rem' }}>
                    Covers {story.words_covered.length} Vocabulary Words
                  </span>
                  <div style={{
                    display: 'flex',
                    flexWrap: 'wrap',
                    ...
                    {story.words_covered.slice(0, 5).map((w, wIdx) => (
                      <span key={wIdx} style={{ fontSize: '0.65rem', padding: '0.1rem 0.35rem', borderRadius: '4px', backgroundColor: 'hsl(var(--bg-canvas))', color: 'hsl(var(--text-secondary))', border: '1px solid hsl(var(--border-muted))' }}>
                        {w.toUpperCase()}
                      </span>
                    ))}
                    {story.words_covered.length > 5 && (
                      <span style={{ fontSize: '0.65rem', padding: '0.1rem 0.35rem', color: 'hsl(var(--text-muted))' }}>
                        +{story.words_covered.length - 5} more
                      </span>
                    )}
                  </div>
                </div>
      ...
      {activeStory && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {showBengali && (
            <div className="card animate-fade" style={{ padding: '2rem', borderLeft: '4px solid hsl(var(--secondary))', backgroundColor: 'hsl(var(--bg-surface) / 0.5)' }}>
              ...
              <p style={{ fontSize: '1.1rem', color: 'hsl(var(--text-secondary))', lineHeight: '1.8' }}>
            ...
            <div className="glass-panel animate-slide-up" style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', textAlign: 'left' }}>
                ...
                {selectedWord.bengali_meaning && (
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'hsl(var(--text-muted))', fontWeight: '700', textTransform: 'uppercase' }}>Bengali Meaning</div>
                    <p style={{ fontSize: '0.95rem', color: 'hsl(var(--primary))', fontWeight: '500' }}>{selectedWord.bengali_meaning}</p>
                  </div>
                )}
                {selectedWord.mnemonic && (
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'hsl(var(--text-muted))', fontWeight: '700', textTransform: 'uppercase' }}>Mnemonic Aid</div>
                    <p style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))', lineHeight: '1.4' }}>{selectedWord.mnemonic}</p>
                  </div>
                )}
              </div>
            </div>
